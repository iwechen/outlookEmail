...
class OutlookScheduler(object):
    ...
    def time_str_to_dtime(self,timeStr):
        ...
        ltime = time.localtime(time.mktime(time.strptime(timeStr, "%b %d, %Y %H:%M")))
    ...
    def content_collec(self):
        ...
        while self.flag:
            ...
            if order_li_one == []:
                ...
                time_dic = self.time_str_to_dtime(re.search(r'\,\s{0,2}(\w{3}\s+\d+\,\s+\d+\s+\d+\:\d+)\s+[A,P,M]{2}',content).group(1))
                # 客户姓名
                buyer_str = re.sub(r'<.*?>|\r|\n','',re.findall(r'Buyer(.*?)Note',content,re.S)[0])
                # 客户邮箱
                buyer_email = re.findall(r'sent to\s{0,1}(.*?\.com)',content)[0]
                # 产品编号
                item_li =list()
                # 产品金额
                price_li = list()
